					...
					<table id="example1" class="table table-bordered table-striped">
						...
						<tbody>
							<tr ng-repeat="menu in menus">
								...
								<td>{{menu.isview }}</td>
								<td>
									<a href="#/user-update/{{menu}}" type="button" class="btn btn-info">编辑</a>
									<a href="#/user-delete/{{menu.id}}" type="button" class="btn btn-info">删除</a>
								</td>
							</tr>
						</tbody>
					</table>
		...
		<div class="row">
			<div class="col-sm-5">
				<div class="dataTables_info" id="example1_info" role="status" aria-live="polite">
					...
					<select ng-model="dataNum" ng-options="x for x in selectedNum" ng-init="dataNum=selectedNum[0]"></select>
					条记录
				</div>
			</div>
			<div class='col-sm-2'>
				第
				<select style="width: 35px; height: 24px; margin: 0px 2px;" ng-model="$parent.pageNum" ng-init="pageNum=pageNums[0]" ng-options="x for x in pageNums"></select>
				页
			</div>
			<div class="col-sm-5">
				<ul class="pagination pull-right" style="margin: 8px;">
					...
					<li ng-if="p_current!=1"><a href="javascript:void(0);" ng-click="p_pre()">上一页</a></li>

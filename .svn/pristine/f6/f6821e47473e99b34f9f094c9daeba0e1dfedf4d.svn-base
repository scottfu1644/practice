...
package com.randstad.rest.diagram.services;
...
public class ProcessInstanceHighlightsResource {
  ...
  public ObjectNode getHighlighted(@PathVariable String processInstanceId) {
    ...
    try {
      ...
      ProcessDefinitionEntity processDefinition =
          (ProcessDefinitionEntity) repositoryService.getProcessDefinition(processInstance
   ...
   * getHighlightedFlows
   * 
   * code logic: 1. Loop all activities by id asc order; 2. Check each activity's outgoing
   * transitions and eventBoundery outgoing transitions, if outgoing transitions's destination.id is
   * in other executed activityIds, add this transition to highLightedFlows List; 3. But if activity
   * is not a parallelGateway or inclusiveGateway, only choose the earliest flow.
   * 
   * @param activityList
   ...
   * @param activity
   * @param hisActInstList
   * @param isExclusive if true only return one flowId(Such as exclusiveGateway, BoundaryEvent On
   *        Task)
   * @return
   */
